...
// Save / load (simple text file)
void saveCharacter(const Character *c, const char *filename) {
    FILE *f = fopen(filename, "w");
    if (!f) {
        printf(RED "Failed to save game.\n" RESET);
        return;
    }
    fprintf(f, "%s\n", c->name);
    fprintf(f, "%s\n", c->element);
    ...
    fprintf(f, "%d\n", c->talents.tier);
    fclose(f);
    printf(GREEN "Game saved to %s.\n" RESET, filename);
}

int loadCharacter(Character *c, const char *filename) {
    FILE *f = fopen(filename, "r");
    if (!f) {
        printf(RED "No save file found (%s).\n" RESET, filename);
        return 0;
    }
    if (!fgets(c->name, sizeof(c->name), f)) { fclose(f); return 0; }
    c->name[strcspn(c->name, "\n")] = 0;

    if (!fgets(c->element, sizeof(c->element), f)) { fclose(f); return 0; }
    c->element[strcspn(c->element, "\n")] = 0;

    if (!fgets(c->role, sizeof(c->role), f)) { fclose(f); return 0; }
    c->role[strcspn(c->role, "\n")] = 0;

    if (fscanf(f, "%d", &c->level) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->maxHP) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->hp) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->attack) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->defense) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->xp) != 1) { fclose(f); return 0; }
    if (fscanf(f, "%d", &c->talents.tier) != 1) { fclose(f); return 0; }

    fclose(f);
    printf(GREEN "Loaded character: %s (Level %d)\n" RESET, c->name, c->level);
    return 1;
}

// Generate enemy by stage (with boss tiers)
Character generateEnemy(int stage) {
    ...
    if (stage == 2) {
        strcpy(e.name, "Grimfang Captain");
        strcpy(e.element, "Shadow");
        strcpy(e.role, "Boss");
        e.level = 2;
        e.maxHP = 50;
        e.hp = e.maxHP;
        ...
        strcpy(e.name, "Dreadspire Warlord");
        strcpy(e.element, "Shadow");
        strcpy(e.role, "Boss");
        e.level = 4;
        e.maxHP = 70;
        ...
        strcpy(e.name, "Umbravore Sovereign");
        strcpy(e.element, "Shadow");
        strcpy(e.role, "Boss");
        e.level = 5;
        e.maxHP = 90;
        ...
        strcpy(e.name, "Umbral Stalker");
        strcpy(e.element, "Shadow");
        strcpy(e.role, "Minion");
        e.level = stage;
        e.maxHP = 30 + stage * 5;
        e.hp = e.maxHP;
        e.attack = 6 + stage;
...
// Single battle; returns 1 if player survived, 0 if died or fled
int runBattle(Character *player, int stage) {
    Character enemy = generateEnemy(stage);
    Status pStatus = {0};
    Status eStatus = {0};
    int guard = 0;

    printf(GREEN "\nYou enter Stage %d as %s (%s, %s).\n" RESET,
           stage, player->name, player->element, player->role);
    ...
    while (player->hp > 0 && enemy.hp > 0) {
        ...
        if (choice == 1) {
            ...
            printf(YELLOW "You fled from Stage %d.\n" RESET, stage);
            return 0;
        } else {
    ...
    if (player->hp <= 0) {
        printf(RED "\nYou were defeated on Stage %d.\n" RESET, stage);
        return 0;
    } else if (enemy.hp <= 0) {
        printf(GREEN "\nStage %d cleared!\n" RESET, stage);
        int baseXP = 70 + rand() % 41;
        if (strcmp(enemy.role, "Boss") == 0) baseXP += 40;
        player->xp += baseXP;
        printf(CYAN "%s gained %d XP!\n" RESET, player->name, baseXP);
        levelUp(player);
        ...
        if (player->hp < player->maxHP) {
            player->hp += 6;
            if (player->hp > player->maxHP) player->hp = player->maxHP;
            printf(GREEN "You catch your breath between battles (+6 HP).\n" RESET);
        }

        // autosave
        saveCharacter(player, "save.txt");
        return 1;
    }
...
int main(void) {
    ...
    while (running) {
        printf(BLUE "\n=== MAIN MENU ===\n" RESET);
        printf("1) Start Campaign (5 stages)\n");
        printf("2) View Talent Tree\n");
        printf("3) Load Saved Character\n");
        printf("4) Save Character\n");
        printf("5) Exit\n");
        printf("Enter choice: ");
        int menu;
        if (scanf("%d", &menu) != 1) {
        ...
        if (menu == 2) {
            if (hasPlayer) showTalentTree(&player);
            else printf("No character loaded.\n");
        }
        else if (menu == 3) {
            if (loadCharacter(&player, "save.txt")) {
                hasPlayer = 1;
            }
        }
        else if (menu == 4) {
            if (hasPlayer) saveCharacter(&player, "save.txt");
            else printf("No character to save.\n");
        }
        else if (menu == 5) {
            printf("Goodbye.\n");
            break;
            ...
            printf(GREEN "\nStarting a 5-stage campaign!\n" RESET);
            int stage;
            int alive = 1;
            for (stage = 1; stage <= 5; stage++) {
                if (!runBattle(&player, stage)) {
                    alive = 0;
                    break;
                }
            }
            ...
            if (alive) {
                printf(GREEN "\nYou cleared all 5 stages! Legendary.\n" RESET);
            } else {
                printf(RED "\nYour run has ended.\n" RESET);
            }
            ...
            if (player.hp > 0) {
                if (player.hp < player.maxHP) {
                    ...
                    printf(GREEN "You rest back in town and recover to full HP.\n" RESET);
                }
            }
        }
        else {
